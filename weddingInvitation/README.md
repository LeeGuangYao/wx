...
每章使用 `100dvh` 最小高度（回退至 `100svh` / `100vh`）。使用原生 `proximity` 滚动吸附和页脚锚点翻页；短屏、横屏或放大文字时，内容可以撑高页面，避免强制整屏吸附影响阅读。桌面保持居中的邀请函版面，最大宽度 560px。
...
每张提供长边 960px / 1600px 两档 WebP（质量 90）及 1600px 渐进 JPEG 回退（质量 90、4:4:4）。竖图实际宽度分别为 720px / 1200px，横幅宽度分别为 960px / 1600px；`srcset` 使用实际宽度。图片已校正 EXIF 方向、转换为 sRGB 并去掉拍摄元数据。保留原片，网页版本采用适合屏幕显示的缩放和高质量有损编码，并非逐像素无损。

七张原片合计约 50.5 MiB，1600px WebP 合计约 1.13 MiB，减少约 97.8%；960px WebP 合计约 530 KiB。浏览器按显示尺寸和像素密度选择其中一档，支持 WebP 时不会下载 JPEG 回退。封面优先加载，其余使用原生懒加载和异步解码。已对照同尺寸原片裁片检查人脸与婚纱细节，没有发现明显偏色或压缩伪影。

旧封面 `src/assets/images/wedding-cover.jpg` 和已有分享缩略图 `public/share-cover.jpg` 保留不变；页面现通过 `photos.ts` 使用新资源，更换网页封面请更新 `photos/cover-*` 和 `photos/cover.jpg`。替换后检查人物裁切、文字可读性和横向溢出。
